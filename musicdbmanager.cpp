...
bool MusicDbManager::addOneMusic(MusicInfoData *data)
{
    ...
    QString queryStr=QString("INSERT INTO all_music_info(title,artist,album,duration,favourite,size) VALUES('%1','%2','%3','%4',%5,%6);")
            .arg(title)
            .arg(artist)
            .arg(album)
            .arg(duration)
...
{   QList<MusicInfoData *> musciFavouriteList;
    QSqlQuery query;
    QString queryStr=QStringLiteral("SELECT * FROM all_music_info WHERE favourite=1 ORDER BY title,album ASC;");
